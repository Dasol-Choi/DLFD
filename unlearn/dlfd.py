...
from evaluation.mia import MIA, MIA_training_monitor
...
def train_dlfd(dlfm_model, data_loaders, criterion, optimizer, device, perturbation_steps, perturbation_strength, forget_threshold, max_forgetting_score, initial_retain_weight, final_retain_weight, forget_weight, val_sample, path):
   ...
   for batch_idx, (x_retain, y_retain) in enumerate(data_loaders['retain_test']):
       ...
       mia = MIA_training_monitor(dlfm_model, data_loaders["retain_test"], 
                              data_loaders['forget_test'], data_loaders['val'], device, max_samples=val_sample)
       forgetting_score = mia["Forgetting Score"]
       final_score = (test_acc["Acc"] + 1 - mia["Forgetting Score"] * 2) / 2 

       print(f'Batch {batch_idx + 1}/{len(data_loaders["retain_test"])} - '
             f'Test Acc: {test_acc["Acc"]:.4f}, '
             f'MIA Forgetting Score: {forgetting_score:.4f}, '
   ...
   final_test_acc = calculate_accuracy(dlfm_model, data_loaders['val'], criterion, device)
   final_mia = MIA(dlfm_model, data_loaders["retain_test"], 
                   data_loaders['forget_test'], data_loaders['val'], device)
   final_forgetting_score = final_mia["Forgetting Score"]
   final_score = (final_test_acc["Acc"] + 1 - final_mia["Forgetting Score"] * 2) / 2 
   print(f'Final Test Acc: {final_test_acc["Acc"]:.4f}, '
         f'Final MIA Forgetting Score: {final_forgetting_score:.4f}, '
         f'Final Score: {final_score:.4f}')
   
   return {
       'final_test_acc': final_test_acc["Acc"],
       'final_forgetting_score': final_forgetting_score,
